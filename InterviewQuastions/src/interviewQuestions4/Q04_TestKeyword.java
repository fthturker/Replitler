...
public class Q04_TestKeyword {
     /*
    ogrencilerin test sonuclarini degerlendiren Java kodu-nu method kullanarak yaziniz
...
*/
    ...
    private static void testSonucu(char[][] answers, char[] keys) {
        for (int i = 0; i < answers.length; i++) { // her bir ogrenciye bakar ogrenci --> i
            ...
            for (int j = 0; j < answers[i].length; j++) { // her ogrencinin cevaplarina bakiyor ogrenci cevaplari --> j
                if (answers[i][j]==keys[j]){ // cevap anahtari ile ogrencı cevaplarını karsılastırıyor cevaplar ->j oldugu icin keys[j] olmali
                    sayac++;
                }
